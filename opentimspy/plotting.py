...
def float_to_idx(
    A: npt.NDArray[np.float64], min_val: np.float64, resolution: np.float64, **kwargs
):
    return np.uint32((A - min_val) * (1.0 / resolution) + 0.5)


def int_to_idx(A: npt.NDArray[np.uint32], min_val: np.uint32, **kwargs):
    if min_val == 0:
        return A
    return A - min_val
...
def mk_bitmap(
    ...
    IMG = make_dense(
        ...
        YI,
        I,
        no_idxes(xax_min, xax_max, xax_res),
        no_idxes(yax_min, yax_max, yax_res),
    )
...
def do_plot(
    ...
    plt.imshow(
        ...
        vmax=max_intens,
        extent=(
            xax_min - resolution_offset(xax_res),
