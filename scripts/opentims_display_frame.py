...
parser = argparse.ArgumentParser(
    description="Display a plot of a single frame from TDF file."
)
parser.add_argument("path", help="TDF dataset path", type=Path)
parser.add_argument(
    "frames",
    help="Comma-separated list of frames, including ranges. Example: 314,320-330,435. Will plot everything if omitted.",
    nargs="?",
    default=None,
)
parser.add_argument(
    "-s",
    "--save",
    help="Save the images to files instead of displaying them",
    action="store_true",
)
...
parser.add_argument(
    "-p",
    "--processes",
    help="Number of subprocesses to use. Will use as many as there are detected cores in your system if omitted.",
    type=int,
    default=None,
)
parser.add_argument(
    "--mz-range",
    help="Custom mz range, example: 400.0-600.0",
    type=partial(rangeize, conversion=float),
    default=Range(0, 2000),
)
parser.add_argument(
    "--scan-range",
    help="Custom scan range, example: 200-300",
    type=partial(rangeize, conversion=int),
    default=None,
)
parser.add_argument(
    "--mz-resolution",
    help="Custom mz binning resolution for plot. Default: 1.0",
    type=float,
    default=1.0,
)
parser.add_argument(
    "--intensity",
    help="Clamp all intensities below this threshold to 0 (for simple noise removal)",
    type=int,
    default=0,
)
parser.add_argument(
    "-o", "--output", help="Output directory if using -s", type=Path, default=Path(".")
)
parser.add_argument(
    "-t",
    "--transform",
    help="Transform the intensities before plotting",
    type=str,
    default="",
    choices="log10 sqrt id".split(),
)
parser.add_argument(
    "-m",
    "--movie",
    help="Create a movie out of saved frames, save it to provided path",
    type=Path,
    default=None,
)
...
if args.movie and not args.save:
    print(
        "If -m/--movie is present, then -s/--save is also required, and -o strongly suggested"
    )
    sys.exit(1)
...
with OpenTIMS(args.path) as OT:
    ...
    def worker(frame_id):
        frame = OT.query(frame_id, columns=["scan", x_column, "intensity"])
        plotting.do_plot(
            plt,
            frame,
            axes=[x_column, "scan"],
        ...
        if args.save:
            plt.savefig(
                args.output / f"frame_{frame_id:06d}.png",
                dpi=600,
                bbox_inches="tight",
                pad_inches=0,
            )
...
if args.movie:
    ...
    subprocess.run(
        f"""ffmpeg -y -framerate 10 -pattern_type glob -i '{args.output / '*.png'}' -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2"  -c:v libx264 -r 50 -pix_fmt yuv420p {args.movie}""",
        shell=True,
    )
