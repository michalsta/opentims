...
parser = argparse.ArgumentParser(
    description="Display a sum plot of select frames of a TDF file."
)
parser.add_argument("path", help="TDF dataset path", type=Path)
parser.add_argument(
    "frames",
    help="Comma-separated list of frames, including slices. Example: 35,12::17 will sum frames: 35 and every 17th frame starting from 12 till the end.",
    default=":",
)
parser.add_argument(
    "-s",
    "--save",
    help="Save the image to file instead of displaying them",
    action="store_true",
)
...
parser.add_argument(
    "--rt-range",
    help="Additionally clamp to RT range (constraining further the frames argument), example: 400.0-600.0",
    type=partial(rangeize, conversion=float),
    default=None,
)
parser.add_argument(
    "--scan-range",
    help="Custom scan range, inclusive, example: 200-300",
    type=partial(rangeize, conversion=int),
    default=None,
)
parser.add_argument(
    "--frame-range",
    help="Custom frame range, inclusive, example: 200-300. It will further constrain the <frames> argument.",
    type=partial(rangeize, conversion=int),
    default=None,
)
parser.add_argument(
...
with OpenTIMS(args.path) as OT:
    ...
    if not (args.rt_range is None):
        frames = set(
            ...
            if args.rt_range.min
            <= OT.frame_properties[frameid].Time
            <= args.rt_range.max
        )
